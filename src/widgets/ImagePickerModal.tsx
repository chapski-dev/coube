...
const ImagePickerModal = forwardRef<BottomSheetModal, ImagePickerModalProps>((
  { modalClose, setPickerResponse }
  , ref) => {
  const { colors, insets } = useAppTheme();

  const openGalary = useCallback(() => {
    modalClose()
    const options: any = { includeBase64: true, mediaType: 'photo', selectionLimit: 1, };
    launchImageLibrary(options, (val) => val?.assets ? setPickerResponse(val) : null);
  }, []);

  const openCamera = useCallback(() => {
    modalClose()
    const options = { includeBase64: true, mediaType: 'photo', saveToPhotos: true, };
    launchCamera(options, (val) => val?.assets ? setPickerResponse(val) : null);
  }, []);
