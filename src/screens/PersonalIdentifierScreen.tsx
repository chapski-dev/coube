...
const PersonalIdentifier = ({
  navigation,
}: ScreenProps<'personal-identifier'>) => {
  ...
  const handleAddPhoto = (val: ImagePickerResponse | null) => {
    ...
    val?.assets &&
      regService.setPersonalIdentifierPhotoBase64(
        val.assets[0].base64 as string,
      );
    val?.assets &&
      regService.setPersonalIdentifierPhotoUri(val.assets[0].uri as string);
  };
        ...
        <Box gap={8} mb={31}>
          ...
          >
            ...
            <Text
              fontWeight="700"
              uppercase
              children="Сергей Крылов дмитриевич"
            />
          </Box>
          <Box
            gap={4}
